...
/ / Get the pointer to the beginning of the Nth item
...
// Get process proc virtual memory
...
// Obtain the CPU and Mem usage of a process.

#define VMRSS_LINE 22
#define pid_t  int

int static getphymem(const pid_t p) {
	char file[64] = { 0 };
	FILE *fd;
...
int static getrmem(pid_t p){
	return getphymem(p);
}


int static gettotalmem(){
	...
	int totalmem;
	char*ret = fgets(line_buff, sizeof(line_buff), fd);
	sscanf(line_buff, "%s %d", name, &totalmem);
	fclose(fd);
	return totalmem;
}
...
bool CBlockTreeDB::LoadBlockIndexGuts()
{
   ...
   /* BlockMap::iterator it = mapBlockIndex.begin();
    while (it != mapBlockIndex.end()){it++;}
    std::cout << "LoadBlockIndexGuts1:"
              << mapBlockIndex .size()
              <<"...."
              << getrmem(getpid())
              << std::endl;

    std::cout << "LoadBlockIndexGuts2:"
              << getrmem(getpid())
              << std::endl;*/
